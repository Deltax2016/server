...
export default class Grid9 extends Component {
    ...
    render() {
        ...
        return (
          ...
          <Grid key='1' item lg={4}>
            {
                this.props.remoteStream.length > 8 ? <Button onClick={this.state.enabled[8]==false ? () => this.handleEnableAudio(8) : () => this.handleDisableAudio(8)}><RemoteVideoView stream={this.props.remoteStream[8]} upload={this.props.upload} id={'remoteview'} sound={this.state.enabled[8]} num={9}/> </Button> : null
          }
              </Grid>
          </Grid>
          </div>
